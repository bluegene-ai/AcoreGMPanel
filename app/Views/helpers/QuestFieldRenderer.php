...
class QuestFieldRenderer
{
    public static function renderGroup(string $groupKey, array $groupCfg, array $fieldsCfg, array $quest, ?string $tab = null): string
    {
        ...
        foreach($groupCfg['fields'] as $fieldName){
            if(!isset($fieldsCfg[$fieldName])) continue; $f=$fieldsCfg[$fieldName];
            $html .= self::renderField($fieldName,$f,$quest);
        }
        $html .= '</div></section>';
        return $html;
    }
    ...
    private static function renderField(string $name, array $cfg, array $quest): string
    {
        ...
        $label = $cfg['label'] ?? $name; $type = $cfg['type'] ?? 'string';
        $help = $cfg['help'] ?? ''; $required = !empty($cfg['required']);
        ...
        if($type==='text'){
            ...
            $extra=''; if(!empty($cfg['bitmask'])||$type==='bitmask'){ $extra=' data-bitmask="'.htmlspecialchars($cfg['mask']??$name).'"'; $inputType='text'; }
            $html .= '<input type="'.$inputType.'" class="form-control form-control-sm" value="'.htmlspecialchars((string)$val).'"'.$commonAttr.$extra.' />';
        }
        if($help) $html .= '<div class="form-text small">'.htmlspecialchars($help).'</div>';
        $html .= '</div>';
        return $html;
    }
}
