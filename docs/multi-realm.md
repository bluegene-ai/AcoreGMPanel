...
- [ ] `php tools/verify_auctionator_listing.php [区索引]` —— GM 上架参数回归（只读，不写库不发 SOAP）：
      ...
      以及页面两张卡片的模式与整组推导价渲染。
- [ ] `php tools/verify_auctionator_filters.php [区索引]` —— 物品筛选回归（只读或状态等价，不写库不发 SOAP）：
      品质白名单 8 行与越界/缺失校验、类别按类分组与「整类配额」端点、类别 16（雕文）不再被
      旧范围挡掉、缺失类别不再静默落到类别 0，以及页面上的品质/类别卡片与买断开关渲染。
