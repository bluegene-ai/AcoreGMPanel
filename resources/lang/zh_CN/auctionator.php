<?php
/**
 * File: resources/lang/zh_CN/auctionator.php
 * Purpose: 拍卖机器人（mod-auctionator）管理端文案。
 */

return [
    'page_title' => '拍卖机器人',
    'intro' => '查看 mod-auctionator 的运行状态与挂单、在线编辑模块配置、维护物品策略表，并通过 SOAP 执行模块自带的 GM 命令。',
    'intro_short' => '查看挂单与市场数据、在线改配置、维护物品策略表，并执行模块 GM 命令。',
    'scope_note' => '当前服务器：:server · 配置文件：:path',
    'tabs' => [
        'label' => '拍卖机器人页面分区',
        'status' => '状态',
        'settings' => '配置',
        'policy' => '物品策略',
        'actions' => '操作',
    ],
    'state' => [
        'on' => '开',
        'off' => '关',
        'listed' => '上架',
        'not_listed' => '不上架',
        'buyout_on' => '买断模式：开（有买断价，也可以竞拍）',
        'buyout_off' => '买断模式：关（只保留竞价）',
    ],
    // 物品类别（item_template.class）的中文名，用于两张筛选卡片；缺哪个就用模块标签表里的英文名。
    'types' => [
        0 => '消耗品',
        1 => '容器',
        2 => '武器',
        3 => '宝石',
        4 => '护甲',
        5 => '材料',
        6 => '弹药',
        7 => '商品',
        8 => '通用',
        9 => '配方',
        10 => '货币',
        11 => '箭袋',
        12 => '任务物品',
        13 => '钥匙',
        14 => '永久物品',
        15 => '杂项',
        16 => '雕文',
    ],
    // item_template.quality
    'qualities' => [
        0 => '粗糙',
        1 => '普通',
        2 => '优秀',
        3 => '精良',
        4 => '史诗',
        5 => '传说',
        6 => '神器',
        7 => '传家宝',
    ],
    'houses' => [
        'alliance' => '联盟 (2)',
        'horde' => '部落 (6)',
        'neutral' => '中立 (7)',
    ],
    // 上架模式：决定“起拍单价 / 买断单价”这两个参数怎么用。
    'modes' => [
        'buyout' => '一口价',
        'bid' => '竞拍',
        'legacy' => '跟随全局',
        'buyout_hint' => '一口价（不竞拍）',
        'bid_hint' => '竞拍（起拍价 + 可选买断价）',
    ],
    'master' => [
        'title' => '模块状态',
        'power_title' => '本区机器人总开关（立即生效）',
        'power_start' => '启动本区机器人',
        'power_stop' => '停止本区机器人',
        'power_hint' => '只影响当前选中的区服：先把 Auctionator.Enabled 写进本区的 mod_auctionator.conf（重启后仍保持），再向本区 worldserver 发 .auctionator start / stop 让它立刻生效。各区互不影响。',
        'power_hint_short' => '写回本区配置（重启后保持），其他区不受影响。',
        'buyout_title' => '买断模式（立即生效）',
        'buyout_on' => '开启买断',
        'buyout_off' => '关闭买断（只保留竞价）',
        'buyout_conf' => '（:key = :value）',
        'buyout_hint' => '控制自动卖家新建条目的形态，同样两步：把 Auctionator.Seller.BidOnly 写进本区 conf（重启后仍保持），再向本区 worldserver 发 .auctionator buyout 0|1，卖家下一轮就按新值上架。开启买断 = BidOnly 0：条目带一口价，起拍价由 Auctionator.Seller.BidStartModifier 推导；关闭买断 = BidOnly 1：完全不设一口价，算出来的价格直接成为起拍价，只能靠竞拍成交（无人出价则过期回收）。只影响本区；条目策略里显式选了“一口价/竞拍”的 gm_list 行不受影响。',
        'buyout_hint_short' => '卖家下一轮生效；清单里指定模式的行不受影响。',
    ],
    'listings' => [
        'title' => '拍卖行挂单',
        'total' => '挂单总数',
        'bot' => '机器人挂单',
        'player' => '玩家挂单',
        'bid_only' => '纯竞价条目',
        'with_buyout' => '带一口价条目',
        'distinct_items' => '涉及物品种类',
        'expiry_window' => '最早/最晚到期',
        'bot_mail' => '机器人邮箱邮件',
        'unavailable' => '无法读取 auctionhouse 表，请确认角色数据库连接与表结构。',
        'not_deployed' => '本区没有部署 mod-auctionator，因此没有 auctionhouse 数据可统计。',
    ],
    'market' => [
        'title' => '市场数据表',
        'rows' => '数据行',
        'items' => '物品种类',
        'fresh_items' => '仍有效种类',
        'newest' => '最新扫描',
        'oldest' => '最旧扫描',
        'missing_table' => '角色库里没有 mod_auctionator_market_price 表，自动卖家会回退到 item_template.BuyPrice 定价。',
        'unreadable' => '市场数据表读取失败，详见 storage/logs/auctionator_repository_warnings.log。',
        'not_deployed' => '本区没有部署 mod-auctionator，因此没有市场数据表。',
        'hint' => '价格只有在扫描时间新于 Auctionator.MarketData.MaxAgeDays 时才会被采用。',
        'hint_short' => '只采用仍在时效内的扫描价。',
        'retention_days' => '保留天数',
    ],
    'log' => [
        'title' => '模块日志（尾部）',
        'empty' => '日志文件不存在或为空；模块启用并在世界循环里跑过一次后才会有内容。',
    ],
    'settings' => [
        'title' => '模块配置（mod_auctionator.conf）',
        'path' => '写入目标：:path',
        'missing' => '配置文件不存在',
        'read_only_file' => '配置文件不可写',
        'restart_note' => '模块只在 worldserver 构造 Auctionator 单例时读取配置，保存后必须重启 worldserver 才生效（可在“守护管理”页重启）。写入前会自动备份为 .agmp.bak，注释与未知键保持不变。',
        'restart_note_short' => '保存后必须重启 worldserver 才生效。',
        'save' => '保存配置',
        'backup_note' => '仅写入有变化的键；未改动的行不会重写。',
        'backup_note_short' => '仅写入有变化的键。',
    ],
    'groups' => [
        'master' => '总开关与身份',
        'alliance_seller' => '联盟卖家',
        'horde_seller' => '部落卖家',
        'neutral_seller' => '中立卖家',
        'seller_common' => '卖家通用',
        'alliance_bidder' => '联盟竞拍者',
        'horde_bidder' => '部落竞拍者',
        'neutral_bidder' => '中立竞拍者',
        'bidder_common' => '竞拍者通用',
        'market' => '市场数据',
        'multipliers_seller' => '卖家品质倍率',
        'multipliers_bidder' => '竞拍者品质倍率',
        'other' => '其他',
    ],
    'fields' => [
        'enabled' => '启用',
        'bid_only' => '纯竞价（关闭买断模式；等于界面上的“关闭买断”）',
        'character_id' => '账号 ID（虚拟会话）',
        'character_guid' => '拍卖角色 GUID',
        'max_auctions' => '该拍卖行挂单上限',
        'cycle_minutes' => '周期（分钟）',
        'auctions_per_run' => '每周期上架数量',
        'default_price' => '默认基础价（铜）',
        'randomize_stack_size' => '随机堆叠数量',
        'bid_start_modifier' => '起拍价下浮比例',
        'prefer_market_items' => '优先有市场数据的物品',
        'exclude_unverified_items' => '排除 VerifiedBuild=1 的物品',
        'min_price_modifier' => '价格下限倍率',
        'max_price_modifier' => '价格上限倍率',
        'max_per_cycle' => '每周期购买上限',
        'bid_on_own' => '允许竞拍自己的条目',
        'max_age_days' => '市场数据最大时效（天）',
        'import_file' => '定时导入文件（CSV）',
        'import_source' => '导入来源标记',
        'import_interval_minutes' => '导入检查间隔（分钟）',
        'import_max_rows' => '单次导入行数上限',
        'scan_interval_minutes' => '采样本区拍卖行的间隔（分钟，0 = 只手动）',
        'scan_exclude_self' => '采样时排除机器人自己的挂单',
        'retention_days' => '历史保留天数',
        'multiplier_poor' => '粗糙',
        'multiplier_normal' => '普通',
        'multiplier_uncommon' => '优秀',
        'multiplier_rare' => '精良',
        'multiplier_epic' => '史诗',
        'multiplier_legendary' => '传说',
        'value' => '值',
    ],
    'policy' => [
        'disabled_title' => '永久黑名单',
        'disabled_hint' => 'mod_auctionator_disabled_items：列在这里的 item_template.entry 永不被自动上架。热生效，下一个卖家周期（1 分钟）采用。',
        'disabled_hint_short' => '这里列出的物品永不被自动上架；约 1 分钟后生效。',
        'disabled' => '黑名单',
        'item_id' => '物品 ID',
        'item_name' => '物品名',
        'add' => '加入黑名单',
        'remove' => '移除',
        'empty' => '暂无数据。',
        'itemclass_title' => '类别白名单（自动上架范围）',
        'itemclass_hint' => 'mod_auctionator_itemclass_config：**这张表就是白名单**——自动卖家只会上架这里存在的“类别/子类别”组合，不在表里的类型一律不上架。配额（max_count）是该类物品在同一拍卖行的挂单上限：**填 0 就是这一类永不上架**，填 2 表示同一件物品最多挂 2 条。堆叠（stack_count）是每条挂几个，填 0 表示用物品自身的最大堆叠。每个周期重新读库，改完下一个周期（1 分钟）生效，不用重启。',
        'itemclass_hint_short' => '配额 0 = 该类永不上架；改完立即生效，无需重启。',
        'itemclass_totals' => '当前显示 :shown / 共 :total 行，其中 :listed 行在自动上架',
        'itemclass_class_state' => '（:listed / :rows 个子类别在自动上架）',
        'class_quota' => '整类配额',
        'apply_to_class' => '应用到整类',
        'class_no_rows' => '“:class”目前没有任何行，所以这一类完全不在白名单里（不会上架）。选一个大于 0 的配额再点“应用到整类”，会按模块自带的标签表补齐该类所有子类别。',
        'class_no_rows_short' => '该类不在白名单，不会上架；设好配额再“应用到整类”。',
        'quality_title' => '品质白名单（自动上架范围）',
        'quality_hint' => 'mod_auctionator_quality_config：**不上架**的品质，自动卖家永不上架；**没有行**的品质按“允许”处理，所以这张表为空时行为和以前完全一样。卖家每个周期都会重读它，改完下一轮就生效，不用重启。条目策略里手动上架（GM 精选清单 / GM 上架）不受这里限制。',
        'quality_hint_short' => '无行 = 允许；标为不上架的品质永不上架。',
        'quality' => '品质',
        'quality_items' => 'item_template 条目数',
        'quality_state' => '自动上架',
        'quality_default' => '（无行，按允许处理）',
        'quality_table_missing' => '缺少 mod_auctionator_quality_config 表，品质筛选暂时不可用。请先对本区 world 库执行模块的 data/sql/db-world/updates/2026_09_24_01_quality_config.sql。',
        'table_missing' => '缺少数据表 :table，请先执行模块自带的 SQL 更新。',
        'table_not_deployed' => '本区没有部署 mod-auctionator，以下策略表不可用。',
        'table_unavailable' => '连不上本区数据库，暂时读不到策略表（恢复连接后刷新即可，不用重导 SQL）。',
        'class' => '类别',
        'subclass' => '子类别',
        'bonding' => '绑定门槛',
        'max_count' => '配额',
        'stack_count' => '堆叠',
        'save' => '保存',
        'delete' => '删除',
        'gm_title' => 'GM 精选清单（addlist）',
        'gm_hint' => 'mod_auctionator_gm_list：用一条 “.auctionator addlist” 批量补货的清单。每行的 mode 决定价格怎么用：一口价只填买断单价（起拍价被钉在买断价上，无法低价竞拍）；竞拍填起拍单价，买断单价可留 0（不设一口价）或填得比起拍价高。两个价格都是**单价（铜/个）**，整组挂单 = 单价 × 堆叠。hours 为 1..720；owner 为 0 时金币按模块规则回收。',
        'gm_hint_short' => '每行按自己的模式上架；价格填单价，整组 = 单价 × 堆叠。',
        'mode' => '上架模式',
        'mode_choose' => '— 请选择 —',
        'bid_price' => '起拍单价（铜/个）',
        'buyout_price' => '买断单价（铜/个）',
        'buyout_price_short' => '买断单价（铜/个，0 = 不设）',
        'listing_totals' => '整组挂单（起拍 / 买断）',
        'totals_start' => '起拍',
        'totals_buyout' => '买断',
        'legacy_note' => '价格由 Auctionator.Seller.BidOnly 决定',
        'owner_bot' => '机器人（金币回收）',
        'gm_form_hint' => '填物品 ID 会新增或覆盖该行；整组挂单 = 单价 × 堆叠。',
        'gm_columns_missing' => '本区 mod_auctionator_gm_list 缺少 mode / bid 列，清单暂时读不出来。请先对本区 world 库执行模块的 data/sql/db-world/updates/2026_09_24_00_gm_list_mode.sql。',
        'stack' => '堆叠',
        'hours' => '时长（小时）',
        'house' => '拍卖行',
        'owner' => '收款角色 GUID',
        'enabled' => '启用',
        'enable' => '启用',
        'disable' => '停用',
        'showing' => '当前显示 :shown 行 / 共 :total 行',
        'gm_totals' => '当前显示 :shown 行 / 共 :total 行，其中启用 :enabled 行',
        'filter_from' => '从物品 ID 开始',
        'filter_apply' => '筛选',
    ],
    'actions' => [
        'read_title' => '只读查询',
        'status' => '查询状态（.auctionator status）',
        'market' => '查询市场表（.auctionator market）',
        'runtime_title' => '运行时开关',
        'runtime_hint' => '这些命令只改内存里的配置，重启 worldserver 后回到配置文件的值；总开关 Auctionator.Enabled 无法在运行时打开。',
        'runtime_hint_short' => '只改运行时内存，重启后回到配置文件的值。',
        'enable' => '启用',
        'disable' => '停用',
        'auctionspercycle' => '设置每周期上架数',
        'gm_title' => 'GM 上架',
        'gm_hint' => '补货会把启用中的 mod_auctionator_gm_list 行按各自的模式上架；expireall 只影响机器人自己的条目（勾选后连玩家条目一起取消）。',
        'gm_hint_short' => '补货只上架清单启用行；强制过期默认不动玩家条目。',
        'addlist' => '补货（addlist）',
        'addlist_owner' => '补货时用谁的收款角色',
        'addlist_owner_row' => '每行自己的收款角色',
        'addlist_hint' => '不覆盖收款角色时，命令里不带 owner 参数，模块会保留每行自己的 owner 列；选了 bot 则把所有行的金币都回收。',
        'addlist_hint_short' => '不覆盖 = 每行自己的收款角色；选机器人 = 金币全部回收。',
        'expire_all_included' => '包含玩家条目',
        'expireall' => '强制过期（expireall）',
        'items' => '物品 ID（逗号分隔）',
        'add' => '上架（add）',
        'add_hint' => '自动卖家的条目时长固定为 :hours 小时；只有 GM 上架可以指定时长。',
        'add_hint_short' => '自动卖家的时长固定，只有这里能自定义。',
        'add_owner_hint' => '收款角色填 bot 表示金币回收；填角色 GUID 表示金币邮寄给该角色。面板通过 SOAP 控制台发送命令，没有玩家会话，所以“me”不可用（会被 worldserver 拒绝），故不提供。',
        'add_owner_hint_short' => '填 bot = 金币回收；填角色 GUID = 邮寄给该角色。',
        'market_title' => '市场数据维护',
        'marketimport' => '立即导入 CSV',
        'marketscan' => '采样本区拍卖行 → 生成市场价',
        'marketscan_hint' => '把本区拍卖行当前的挂单聚合成市场价（每个物品一行，单价：拍卖行存的是整组价，这里会除以堆叠数）。有买断价的挂单优先，只有整区都没有买断价时才退回用起拍价；按堆叠量加权平均。默认排除机器人自己的挂单（避免"市场价"变成机器人自己的报价），排除时若本区挂单全是机器人的会写入 0 行并给出说明。定时执行请设 Auctionator.MarketData.ScanIntervalMinutes（0 = 只在手动点按钮/发命令时跑）。',
        'marketscan_hint_short' => '把本区挂单聚合成市场价，默认排除机器人自己的挂单。',
        'marketprune' => '清理历史',
        'market_hint' => '导入需要先在配置里填写 Auctionator.MarketData.ImportFile；清理会删除超过保留期的扫描行。',
        'market_hint_short' => '导入需先在配置里指定 CSV；清理按保留期删除扫描行。',
        'output_title' => '命令输出',
        'output_empty' => '（暂无输出）',
    ],
    'feedback' => [
        'config_saved' => '已写入 :count 个配置键，重启 worldserver 后生效。',
        'config_unchanged' => '没有需要写入的变化。',
        'disabled_added' => '物品 :item 已加入黑名单。',
        'disabled_removed' => '物品 :item 已移出黑名单。',
        'itemclass_saved' => '类别 #:class/#:subclass 已保存。',
        'itemclass_deleted' => '类别 #:class/#:subclass 已删除。',
        'quality_saved' => '品质“:quality”已设为 :state。',
        'buyout_enabled' => ':server 的买断模式已开启：配置已写回，运行中的卖家下一轮就会给新条目加一口价。',
        'buyout_disabled' => ':server 的买断模式已关闭：配置已写回，运行中的卖家下一轮起不再设一口价，只保留竞价。',
        'buyout_offline' => '配置处理结果：:state；但 :server 的 worldserver 没有响应，本次开关未生效（:message）。',
        'gm_saved' => 'gm_list 物品 :item 已保存（模式：:mode）。',
        'itemclass_class_saved' => '“:class”整类配额已设为 :quota（更新 :updated 行，新增 :inserted 行）。',
        'gm_deleted' => 'gm_list 物品 :item 已删除。',
        'gm_toggled' => 'gm_list 物品 :item 的启用状态已切换。',
        'command_success' => '命令已执行。',
        'power_started' => ':server 的拍卖机器人已启动：配置已写回，运行中的模块已立即打开。',
        'power_stopped' => ':server 的拍卖机器人已停止：配置已写回，运行中的模块已立即关闭（已挂出的拍卖不受影响）。',
        'power_runtime_only' => '运行中的机器人已切换，但配置没写成功（:message），重启后会回到旧状态。',
        'power_offline' => '配置处理结果：:state；但 :server 的 worldserver 没有响应，本次开关未生效（:message）。',
        'power_conf_saved' => '本区配置已写入',
        'power_conf_failed' => '本区配置未写入',
    ],
    'errors' => [
        'empty_payload' => '没有提交任何配置项。',
        'invalid_values' => '有 :count 个配置项不合法，未写入任何内容。',
        'nothing_editable' => '没有可写入的配置项。',
        'config_write_failed' => '写入配置失败：:message',
        'config_missing' => '配置文件不存在，无法写入。',
        'config_unreadable' => '配置文件无法读取。',
        'config_not_writable' => '配置文件不可写（权限或路径问题）。',
        'invalid_action' => '无效的操作。',
        'invalid_class' => '类别或子类别不合法。',
        'invalid_quality' => '品质必须在 0..7 之间。',
        'invalid_target' => '无效的开关目标。',
        'invalid_multiplier' => '无效的倍率类型或品质。',
        'invalid_number' => '请输入合法的数字。',
        'item_not_found' => 'item_template 中不存在物品 :item。',
        'not_found' => '目标记录不存在。',
        'items_required' => '请填写至少一个物品 ID。',
        'too_many_items' => '单条命令最多 :max 个物品。',
        'invalid_item' => '物品 ID 不合法：:item',
        'mode_required' => '请选择上架模式（一口价或竞拍）。',
        'buyout_price_required' => '一口价模式需要填写大于 0 的买断单价。',
        'bid_price_required' => '竞拍模式需要填写大于 0 的起拍单价。',
        'buyout_below_bid' => '买断单价（:price）不能低于起拍单价（:bid）。',
        'invalid_owner' => '收款角色只能填 bot 或角色 GUID。',
        'policy_write_failed' => '写入物品策略失败：:message',
        'command_failed' => '命令执行失败。',
        'power_field_missing' => '配置里没有 Auctionator.Enabled 的字段定义，无法切换总开关。',
        'buyout_field_missing' => '配置里没有 Auctionator.Seller.BidOnly 的字段定义，无法切换买断模式。',
        'not_a_number' => '请输入数字。',
        'below_min' => '不得小于 :min',
        'above_max' => '不得大于 :max',
        'too_long' => '长度不得超过 :max 个字符。',
        'invalid_source' => '来源标记只能包含字母、数字、下划线、点或连字符。',
    ],
    'warnings' => [
        'conf_missing' => '配置文件不存在：:path（模块会使用内置默认值，其中总开关默认为 0）。',
        'conf_unreadable' => '配置文件无法读取：:path',
        'server_not_supported' => '当前区服（:server）没有部署 mod-auctionator（本区 world 库里没有模块自己的表），页面为只读。要在本区使用：先把模块 SQL 导进本区的 world / characters 库、在本区目录放一份 configs/modules/mod_auctionator.conf（并配好该区自己的机器人角色），再把本区索引加进 config/auctionator.php 的 supported_server_ids。',
        'server_db_unreachable' => '连不上当前区服（:server）的数据库，无法确认本区是否装了 mod-auctionator，页面暂时只读。请检查 config/generated/servers.php 里这个区的 world / characters 连接（库名、端口、账号）后刷新。',
        'server_denied' => '当前区服（:server）被 config/auctionator.php 的 unsupported_server_ids 明确排除，页面为只读。要让本区可管理，把这个区索引从 unsupported_server_ids 移到 supported_server_ids。',
        'module_disabled' => 'Auctionator.Enabled = 0：模块已加载但事件循环不运行，不会上架、竞拍或导入。',
        'bid_only' => 'Auctionator.Seller.BidOnly = 1：自动卖家与位置式 GM 上架（以及 mode 仍为 legacy 的清单行）都不设一口价，只能靠竞价成交；显式选择了一口价/竞拍的清单行与 add 命令不受影响。',
        'bidder_enabled' => '有竞拍者处于启用状态：机器人会买断玩家拍卖且不付款，核心凭空支付卖家金币（经济水龙头）。',
        'faction_seller_enabled' => '联盟/部落卖家被启用：本服 AllowTwoSide.Interaction.Auction 为 1，它们每个周期只会报错、不会上架。',
        'no_market_data' => '市场数据表为空且未配置导入文件：自动卖家会用 item_template.BuyPrice × 品质倍率定价。',
    ],
    'js' => [
        'modules' => [
            'auctionator' => [
                'confirm' => [
                    'disabled_remove' => '确认把该物品移出自动上架黑名单吗？',
                    'itemclass_delete' => '确认删除该类别行吗？该类别将不再被自动卖家选中。',
                    'itemclass_class_enable' => '确认把该配额应用到这一类的**所有**子类别吗？',
                    'itemclass_class_disable' => '确认把这一类的所有子类别都设为“不上架”吗？自动卖家下一轮起不再补这一类（已挂出的拍卖不受影响）。',
                    'buyout_enable' => '确认开启本区买断模式吗？会写入本区配置文件并让运行中的卖家下一轮就给新条目加一口价。',
                    'buyout_disable' => '确认关闭本区买断模式吗？写入配置后，运行中的卖家下一轮起不再设一口价，只保留竞价（无人出价则过期回收）。',
                    'gm_delete' => '确认删除该 gm_list 行吗？',
                    'expireall' => '确认强制过期这些条目吗？勾选“包含玩家条目”时会连玩家挂单一起取消（物品与出价由核心退还）。',
                    'marketprune' => '确认删除超过保留期的市场扫描数据吗？',
                    'marketimport' => '确认立即导入 CSV 吗？导入在世界线程上执行，文件过大会造成卡顿。',
                    'marketscan' => '确认立即采样本区拍卖行并生成市场价吗？会新增一批市场价行，自动卖家下一轮就会按它们定价（而不是商人价 × 品质倍率）。',
                    'enable' => '确认在运行时启用该开关吗？',
                    'disable' => '确认在运行时停用该开关吗？',
                    'addlist' => '确认按 gm_list 批量补货吗？每行会按自己的模式与价格上架。',
                    'add' => '确认按选定的模式与价格上架这些物品吗？',
                    'power_start' => '确认启动当前区服的拍卖机器人吗？会写入本区配置文件并立即生效。',
                    'power_stop' => '确认停止当前区服的拍卖机器人吗？只会停掉本区，已挂出的拍卖不受影响。',
                ],
                'feedback' => [
                    'config_success' => '配置已保存。',
                    'config_failure' => '配置保存失败。',
                    'restart_required' => '需要重启 worldserver 才会生效。',
                    'policy_success' => '物品策略已更新。',
                    'policy_failure' => '物品策略更新失败。',
                    'action_success' => '命令已执行。',
                    'action_failure' => '命令执行失败。',
                    'power_failure' => '总开关没有生效。',
                    'buyout_failure' => '买断模式开关没有生效。',
                ],
                'actions' => [
                    'output_empty' => '（暂无输出）',
                ],
                // 上架模式与价格的实时预览（JS 自己替换 :占位符）
                'listing' => [
                    'choose_mode' => '请先选择上架模式。',
                    'unit' => ':copper 铜/个',
                    'per_stack' => ':stack 个/组',
                    'buyout' => '一口价：买断 :unit，整组 :total；起拍价被钉在买断价上，无法低价竞拍。',
                    'bid_with_buyout' => '竞拍：起拍 :bid（整组 :bid_total），买断 :buyout（整组 :buyout_total）。',
                    'bid_no_buyout' => '竞拍：起拍 :bid（整组 :bid_total），不设一口价（价高者得）。',
                    'missing_bid' => '请填写大于 0 的起拍单价。',
                    'missing_buyout' => '请填写大于 0 的买断单价。',
                ],
            ],
        ],
    ],
];
