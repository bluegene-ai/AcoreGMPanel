 ...
 * Purpose: 聊天答题（TriviaReward.lua）管理页：状态卡 + 运行控制 + 设置 + 题库 + 奖励预设 + 排行。
 */
...
<section class="tv-panel tv-panel--status" id="tvStatusPanel"
  ...
  <footer class="tv-panel__actions">
    <?php if ($canControl): ?>
      ...
      <button type="button" class="btn outline" data-tv-action="pause"><?= htmlspecialchars(__('app.trivia.actions.pause')) ?></button>
      <button type="button" class="btn outline" data-tv-action="resume"><?= htmlspecialchars(__('app.trivia.actions.resume')) ?></button>
      <button type="button" class="btn outline" data-tv-action="reload"><?= htmlspecialchars(__('app.trivia.actions.reload')) ?></button>
      <button type="button" class="btn outline" data-tv-action="enable"><?= htmlspecialchars(__('app.trivia.actions.enable')) ?></button>
      <button type="button" class="btn outline danger" data-tv-action="disable"
              data-tv-confirm="<?= htmlspecialchars(__('app.trivia.confirm.disable'), ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars(__('app.trivia.actions.disable')) ?></button>
    <?php endif; ?>
  </footer>
</section>
